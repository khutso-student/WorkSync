...
const StatCard = ({ icon, label, value, bgColor, iconColor }) => (
  <div className="flex items-center gap-2 p-1 bg-white border border-[#d3cfcf] w-full h-23 rounded-lg sm:w-45">
...
export default function Overview({
  ...
  return (
    <main className="flex flex-col px-2 md:flex-row justify-center gap-4 w-full h-auto md:px-1">
      {/* Overview Section */}
      <div className="flex flex-col w-full md:w-2/3 px-2 border border-[#ebebeb] rounded-sm">
        <div className="flex justify-between items-center px-1 py-2">
          ...
          <h1 className="text-[#252525] font-semibold hidden sm:block">Weekly</h1>
        </div>
      ...
      {/* Side Section */}
      <div className="flex flex-col h-auto md:w-1/3 p-1 border mb-1 border-[#ebebeb] rounded-sm">
        {/* Recent Employees */}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 p-1">
          {employees.slice(-8).reverse().map((emp) => (
            <div key={emp._id} className="bg-white p-2 shadow rounded-lg border border-gray-200">
